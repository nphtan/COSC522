...
#        all_labels[0,:] = ymodel
        ...
        prob = clf.predict_proba(test_features.T)
        fper, tper, thresh = roc_curve(test_labels, prob[:,1], pos_label=1)
        plot_roc(fper, tper, 'Standardized')
        tp,tn,fn,fp = perf_eval(ymodel, test_labels)
        print('Accuracy:     ', (tp+tn)/(tp+tn+fp+fn))
        print('TP:',tp)
        print('TN:',tn)
...
#        plt.bar([0,1,2]-2*width, knn_sens[:,0], width=width)
#        plt.bar([0,1,2]-width, kmeans_sens[:,0], width=width)
#        plt.bar([0,1,2]+width, wta_sens[:,0], width=width)
#        plt.bar([0,1,2]+2*width, kohonen_sens[:,0], width=width)
#        plt.figure()
#        plt.xticks([0,1,2], ['INF', '1', '2'])
#        plt.xlabel('Norm')
#        plt.ylabel('Specificity')
#        plt.title('Specificity with different norms')
#        plt.bar([0,1,2]-2*width, knn_spec[:,0], width=width)
#        plt.bar([0,1,2]-width, kmeans_spec[:,0], width=width)
#        plt.bar([0,1,2]+width, wta_spec[:,0], width=width)
#        plt.bar([0,1,2]+2*width, kohonen_spec[:,0], width=width)
#        plt.show()
